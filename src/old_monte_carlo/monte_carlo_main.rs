...
#[allow(dead_code)]
pub struct MonteCarloStrategyV1 {
    limit: MonteLimit, c: f64
}

struct MonteCarloChild<G: MonteCarloGame>(G::MOVE, Option<MonteCarloState<G>>);
struct MonteCarloState<G: MonteCarloGame> {
    children: Vec<MonteCarloChild<G>>,
    visited: f64,
    wins: f64,
    leaf_count: u16,
    game: G,
...
impl <G: MonteCarloGame> MonteCarloState<G> {
    fn new(g: G, winner: Option<Winner>) -> Self {
        let children = if winner.is_none() {
            g.moves().into_iter().map(|m| MonteCarloChild(m, None)).collect()
        } else {
            Vec::new()
        };
        Self {
...
impl MonteLimit {
    #[allow(dead_code)]
    pub const fn duration(millis: u64) -> Self {
        let millis = match NonZeroU64::new(millis) {
            None => panic!("invalid millis"),
            Some(v) => v
        };
        Self::Duration { millis }
    }
    #[allow(dead_code)]
    pub const fn times(times: u32) -> Self { Self::Times { times } }
}
...
impl <G: MonteCarloGame + 'static> GameStrategy<G> for MonteCarloStrategyV1 {
    type Carry = ();
...
pub fn make_monte_carlo_move<G: MonteCarloGame + 'static>(g: &G, limit: MonteLimit, c: f64) -> G::MOVE {
    let mut children = {
        ...
        for m in moves.into_iter() {
            ...
            let new_state = MonteCarloState::new(s, w);
...
fn playoff<'a, G: MonteCarloGame+ 'static>(
    mut g: &'a G,
    mut next: &'a mut MonteCarloChild<G>,
    c: f64
) {
    ...
    loop {
        let current = match next.1 {
            ...
            None => {
                ...
                let next_state = MonteCarloState::new(child.0, child.1);
...
fn select_next<'c, 'm, G: MonteCarloGame + 'static>(
    mut children: impl Iterator<Item=&'c mut MonteCarloChild<G>>,
    parent_visited: f64, c: f64,
) -> Option<&'c mut MonteCarloChild<G>> {
