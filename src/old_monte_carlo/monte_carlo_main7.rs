...
impl<G: MonteCarloGame + 'static, W: MultiScoreReducerFactory<G> + ExecutionLimiterFactory<G>> GameStrategy<G> for MonteCarloStrategyV7<G, W> {
...
fn playoff<'a, 'b, G: MonteCarloGame + 'static, W: MultiScoreReducerFactory<G> + ExecutionLimiterFactory<G>>(
    ...
    loop {
        ...
        let current = match next.0 {
            ...
            MonteState::Uncomputed(m, g) => {
                ...
                if let std::ops::ControlFlow::Break(()) = el.next_with_game(n.children.len(), g) {
                    if child.1.is_none() {
                        return;
                    }
                }
    ...
    let mut score_reducer = <W as MultiScoreReducerFactory<G>>::create(wr_config, final_game_state);
